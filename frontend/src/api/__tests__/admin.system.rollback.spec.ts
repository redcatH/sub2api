...
describe('admin system rollback API', () => {
  ...
  it('rollback posts the target version in the request body', async () => {
    ...
    expect(post).toHaveBeenCalledWith('/admin/system/rollback', { version: '0.1.146' }, { timeout: expect.any(Number) })
  ...
  it('rollback without a version posts no body (legacy backup rollback)', async () => {
    ...
    expect(post).toHaveBeenCalledWith('/admin/system/rollback', undefined, { timeout: expect.any(Number) })
  })
})
