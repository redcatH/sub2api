...
func RegisterGatewayRoutes(
	...
	// 之前，避免逐条路由手工维护链导致漏挂。monitorTap 在白名单之前，便于
	// 监控看到被白名单拒绝的请求。
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), monitorTap, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), monitorTap, groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
